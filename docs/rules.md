...
- Related control theme / reference: NIST SP 800-53 AC-2 account management; CIS Microsoft 365 Foundations Benchmark access-control and account-management themes may be useful comparison points. This is a governance reference, not a compliance claim.
...
- Testable acceptance criteria: Given a terminated HR worker with a matching enabled Entra account, the engine creates one R1 finding. Given a terminated HR worker with a matching disabled Entra account, it creates no R1 finding.
...
- Related control theme / reference: NIST SP 800-53 AC-2 account management and AC-6 least privilege; CIS Microsoft 365 Foundations Benchmark access-control and account-management themes may be useful comparison points. This is a governance reference, not a compliance claim.
...
- Testable acceptance criteria: Given a terminated worker matched to an Entra user with an active privileged-role assignment, the engine creates one R2 finding per affected assignment. Given no active or supplied privileged assignment, it creates no R2 finding.
...
- Testable acceptance criteria: Given an enabled contractor account with contract end date before the analysis date, the engine creates one R3 finding. Given a future contract end date or disabled account, it creates no R3 finding.
...
- Unknown status handling: Explicit null `has_mfa_capable_method` values are reported separately as unknown review statuses, not as normal R6 findings and not as clean.
...
- Limitations: This rule does not prove MFA is enforced or not enforced. Current behavior reports explicit null `has_mfa_capable_method` values as unknown, but it does not flag Entra accounts with no matching MFA-registration record at all.
- Related control theme / reference: NIST SP 800-53 IA-2 identification and authentication; CIS Microsoft 365 Foundations Benchmark authentication themes may be useful comparison points. This is a governance reference, not a compliance claim.
...
- Testable acceptance criteria: Given an MFA-registration record with `has_mfa_capable_method = false`, the engine creates one R6 finding. Given `has_mfa_capable_method = true`, it creates no R6 finding. Given an explicit null value, it treats the state as unknown rather than automatically safe.
